//
// Example code for article Codable Conformance for Swift Enums with Multiple Associated Values of Different Types
...
extension ViewState: Decodable {
    init(from decoder: Decoder) throws {
        ...
        case .exchangeHistory:
            let connection = try container.decode(Connection?.self, forKey: .exchangeHistory)
...
let viewStateList = ViewState.list(selectedId: UUID(), expandedItems: [Item(name: "Request 1"), Item(name: "Request 2")])

do {
    let encodedViewStateEmpty = try encoder.encode(viewStateEmpty)
    let encodedViewStateEmotyString = String(data: encodedViewStateEmpty, encoding: .utf8)
    print(encodedViewStateEmotyString ?? "Wrong Data")
    
    
    let decodedViewStateEmpty = try decoder.decode(ViewState.self, from: encodedViewStateEmpty)
